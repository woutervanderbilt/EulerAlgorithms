...
public class Factorizer
{
    ...
    public string FactorToString(long l)
    {
        var sb = new StringBuilder();
        var first = true;
        foreach (var f in Factor(l))
        {

            sb.Append(first ? "" : " * ").Append(f.Item1).Append("^").Append(f.Item2);
            if (first)
            {
                first = false;
            }
    ...
    public long Carmichael(long l)
    {
        ...
        foreach (var tuple in factorization)
        {
            ...
            {
                phi = 1;
                for (int i = 1; i <= tuple.Item2 - (tuple.Item2 <= 2 ? 1 : 2); i++)
                {
                    phi *= 2;
                }
            }

            result = EulerMath.LCM(result, phi);
        }
    ...
    public long PrimitiveRoot(long p)
    {
        for (long r = 2; r < p; r++)
        {
            ...
            {
                var res = new ResidueClass<long>(r, p).ToThePower((p - 1) / factor.Item1);
                if (res.Value == 1)
                {
